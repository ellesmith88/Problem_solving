...
def vary_N_keep_plots(paths):
    ...
    keep = np.arange(1, 30, 1)
    f, (ax1, ax2) = pyplot.subplots(1, 2)
    f.suptitle('Varying N Keep')
    ax1.plot(keep, energies)
    ax2.plot(keep, uncertainties)
    ax1.set_xlabel('N Keep')
    ax1.set_ylabel('Groundstate Energy')
    ax2.set_xlabel('N Keep')
    ax2.set_ylabel('Standard Deviation')
    pyplot.show()

vary_N_keep_plots(paths)
