...
def create_paths(x_array, path_array, n_keep, keep, discard):
...
def groundstate_energy(all_paths, n_keep):
...
def bootstrap(all_paths, n_keep):
...
def uncertainty_calc(path_array, n_keep):
    """calculates the uncertainty using the bootstrap method"""
    energies = []
    for i in range(100):
       sample = bootstrap(path_array, n_keep)
       energies.append(sample)
    mean = np.mean(energies)
    uncertainty = np.std(energies)
    return mean, uncertainty
    
def vary_N_discard_plots(paths):
    """varies the value of a and plots groundstate energy and its uncertainty 
    against the different values of a, where a =T/N"""
    energies = []
    uncertainties = []
    for discard in range(5, 60, 5):
        keep = 4 #frequency at which paths are stored
        #discard = 40 #number of intial paths to discard
        n_keep = int((paths-discard)/keep)
        x_array = np.zeros(N) #array where paths are perturbed
        path_array = np.zeros((n_keep,N)) #array where paths are stored
        paths_array = create_paths(x_array, path_array, n_keep, keep, discard)
        energy_array = groundstate_energy(paths_array, n_keep)[0]
        values = uncertainty_calc(energy_array, n_keep)
        energies.append(values[0])
        uncertainties.append(values[1])
    discard = np.arange(5, 60, 5)
    f, (ax1, ax2) = pyplot.subplots(1, 2)
    f.suptitle('Varying N Discard')
    ax1.plot(discard, energies)
    ax2.plot(discard, uncertainties)
    ax1.set_xlabel('N Discard')
    ax1.set_ylabel('Groundstate Energy')
    ax2.set_xlabel('N Discard')
    ax2.set_ylabel('Standard Deviation')
    pyplot.show()

vary_N_discard_plots(paths)
