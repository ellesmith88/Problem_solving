...
def create_paths(x_array, paths, n_keep):
...
def vary_number_of_paths(x_array):
    ...
    energies = []
    uncertainties = []
    for paths in range(100, 40100, 100):
        n_keep = int((paths-discard)/keep)
        paths = create_paths(x_array,paths, n_keep)
        energy_array = groundstate_energy(paths, n_keep)[0]
        values = uncertainty_calc(energy_array, n_keep)
        energies.append(values[0])
        uncertainties.append(values[1])
    paths = np.arange(100, 40100, 100)
    f, (ax1, ax2) = pyplot.subplots(1, 2)
    f.suptitle('Varying paths')
    ax1.plot(paths, energies)
    ax2.plot(paths, uncertainties)
